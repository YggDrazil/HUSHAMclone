...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Operation Robocop Ultimate,New source added.,5000,"+icon+")")
        if not url is None:
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(Operation Robocop Ultimate,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def _search(url,name):
   # print url,name
    ...
    names = ['Genesis TV','Genesis Movie','The Mutts Nutz','Salts movie','Salts TV',\
             'Youtube','Vimeo']
    dialog = xbmcgui.Dialog()
    index = dialog.select('Choose a video source', names)
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False,regexs=None,reg_url=None,allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from Operation Robocop Ultimate Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to Operation Robocop Ultimate Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from Operation Robocop Ultimate Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to Operation Robocop Ultimate Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(Operation Robocop Ultimate,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Operation Robocop Ultimate,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
