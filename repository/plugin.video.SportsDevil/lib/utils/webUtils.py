...
class DemystifiedWebRequest(BaseRequest):
    ...
    def getSource(self, url, form_data, referer='', xml=False, demystify=False):
        data = enc.smart_unicode(super(DemystifiedWebRequest, self).getSource(url, form_data, referer, xml))
        if not data:
            return None
...
class CachedWebRequest(DemystifiedWebRequest):
    ...
    def getSource(self, url, form_data, referer='', xml=False, ignoreCache=False, demystify=False):
        ...
        if url == self.getLastUrl() and not ignoreCache:
            ...
            data = enc.smart_unicode(super(CachedWebRequest,self).getSource(url, form_data, referer, xml, demystify))
            if data:
                # Cache url
