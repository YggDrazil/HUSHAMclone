...
addon_id='plugin.video.iptvbuddy'
ADDON = xbmcaddon.Addon(id=addon_id)
ADDON_HELPER = Addon(addon_id, sys.argv)

base_url = 'http://iptvbuddy.com'
...
def APICALL(route, params={}):
    ...
    params.update({'sid':site_id, 'language':'3', 'route':route, 'time':timestamp})
    #print params
    import urllib
    import hashlib
...
if ADDON.getSetting('user')=='':
    ...
    if dialog.yesno("Husahm.iptv", "If you dont already have an account, please sign up at:", "[COLOR royalblue]http://iptvbuddy.com[/COLOR]", "", "Exit", "Continue"):
...
cookie_jar = os.path.join(datapath, "Husahm.iptv.lwp")
